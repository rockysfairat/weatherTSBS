...
export const Header = ({ city, country }: LocationData) => {
  ...
  return (
    <header className="flex justify-between items-center px-3 absolute top-0 w-full py-2 backdrop-blur dark:bg-opacity-30 bg-opacity-80 dark:bg-slate-100 bg-zinc-900 text-white">
      {gettingUserLocation(city)}
      <button
        onClick={() => setTheme(colorTheme)}
        className="w-1/6 h-9 dark:border-solid border-solid dark:border-2 border-2 border-slate-50 dark:border-zinc-900 rounded-full px-0.5 py-0.5"
      >
